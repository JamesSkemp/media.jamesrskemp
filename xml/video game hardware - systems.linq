...
XDocument xml = XDocument.Load(@"C:\Users\James\Projects\JamesRSkemp\JamesRSkemp.Media.Web\xml\videogames.xml");

var datum = from x in xml.Descendants("hardware").Descendants("item")
	select new {
		Id = x.Attribute("id").Value,
		Name = x.Element("name").Value
	};

datum.Select (d => d.Id).GroupBy (d => d).Where (d => d.Count () > 1).Dump();
